...
// Role: Senior Mobile Game Developer (Production Specialist)
// 
// Features Included:
// 1. Mobile-First Adaptive Layout: Safe Area (30px top), Level 1 (78% height, Bottom Ad), Level 2 (calc(100%-120px), Top Ad).
// 2. Instructional System: "HOW TO PLAY" modal with animated guide.
// 3. In-game Guidance: Stage 1-1 3-second fade-out tutorial.
// 4. Hardware Support: popstate back-button support, deviceorientation (/30 sensitivity).
// 5. Global SEO & Branding: 100% English UI.
// 6. Audio System: BGM (/bgm.mp3, 0.35 vol), SFX (AudioContext, 1.0 vol).
// ============================================================================
      ...
      <div className="relative w-full max-w-[400px] h-screen max-h-[900px] bg-slate-950 overflow-hidden flex flex-col pt-[30px] shadow-[0_0_50px_rgba(0,0,0,0.8)] sm:rounded-3xl sm:border-4 border-slate-800">
        ...
        {gameState === 'start' && (
          <div className="flex-1 flex flex-col items-center justify-center p-6 bg-slate-950 z-10">
            <button onClick={() => setGameState('tutorial')} className="absolute top-10 right-6 p-2 text-slate-400 hover:text-white transition-colors flex flex-col items-center">
              <Info size={28} />
              <span className="text-[10px] mt-1 font-bold">HOW TO PLAY</span>
            </button>
        ...
        {gameState === 'tutorial' && (
          <div className="absolute inset-0 z-50 bg-slate-950 flex flex-col items-center justify-center p-6 pt-[30px]">
            ...
            <div className="relative w-full max-w-xs h-32 bg-slate-800 rounded-2xl overflow-hidden flex items-center justify-center mb-8 border border-slate-700 shadow-inner">
              <style>{`
                @keyframes rollIn {
                  0% { transform: translateX(-80px) rotate(0deg); }
                  80% { transform: translateX(60px) rotate(360deg); opacity: 1; }
                  100% { transform: translateX(60px) rotate(360deg) scale(0.5); opacity: 0; }
                }
                .animate-roll { animation: rollIn 2.5s infinite ease-in; }
              `}</style>
              <div className="absolute w-14 h-14 bg-black rounded-full right-12 border-2 border-cyan-400 shadow-[0_0_15px_#06b6d4]"></div>
              <div className="absolute text-4xl animate-roll">⚪</div>
            </div>

            <div className="flex flex-col gap-6 w-full max-w-xs bg-slate-900 p-6 rounded-2xl border border-slate-800">
              <div className="flex items-center gap-4"><span className="text-3xl">📱</span><p className="text-sm font-medium text-slate-300">Tilt your phone to hit the target into the black hole!</p></div>
              <div className="flex items-center gap-4"><span className="text-3xl">⚪</span><p className="text-sm font-medium text-slate-300">You control the white ball.</p></div>
              <div className="flex items-center gap-4"><span className="text-3xl">🎯</span><p className="text-sm font-medium text-slate-300">Push the colored target ball into the hole.</p></div>
            </div>
        ...
        {['playing', 'paused', 'clear', 'gameover', 'reward_gate', 'ad_loading'].includes(gameState) && (
          <div className="flex flex-col flex-1 relative bg-slate-950">
            ...
            {/* Level 1 Pro Tips */}
            {isLevel1 && (
              <div className="w-full py-2 bg-slate-800 text-center text-xs text-slate-400 font-medium shrink-0 shadow-inner">
                PRO TIP: Gentle tilts give you better control!
              </div>
            )}

            {/* Game Canvas (Dynamic Height) */}
            <div className={`w-full relative ${isLevel1 ? 'h-[78%]' : 'h-[calc(100%-120px)]'}`}>
              ...
              {gameState === 'playing' && (
                <>
                  ...
                  {currentStage === 1 && (
                    <div className={`absolute inset-0 pointer-events-none transition-opacity duration-1000 ${displayTime < 3 ? 'opacity-100' : 'opacity-0'}`}>
                      <div className="absolute text-xs font-bold bg-blue-600/90 text-white px-3 py-1.5 rounded-full shadow-lg border border-blue-400" style={{ left: positions.player.x, top: positions.player.y - 40, transform: 'translate(-50%, -100%)' }}>
                        Tilt to move me!
                      </div>
                      <div className="absolute text-xs font-bold bg-orange-600/90 text-white px-3 py-1.5 rounded-full shadow-lg border border-orange-400" style={{ left: positions.target.x, top: positions.target.y - 40, transform: 'translate(-50%, -100%)' }}>
                        Put me in the hole!
                      </div>
                    </div>
                  )}
